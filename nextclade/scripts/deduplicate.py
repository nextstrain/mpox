...
def read_accessions(paths: list[str]) -> set[str]:
    accessions = set()
    for path in paths:
...
def process_batch(batch_indices, all_sequences, info_sites, includes):
    """
    ...
    for ying_idx in batch_indices:
        ...
        for yang_idx in range(ying_idx + 1, len(all_sequences)):
            ...
            if yang_seq["id"] not in includes and identical(ying_seq["seq"], yang_seq["seq"], info_sites):
...
def deduplicate(
    ...
    sequences = sorted(
        sequences,
        key=lambda x: (x["id"] in includes, x["number_informative_sites"]),
        reverse=True,
    )
    if sequences:
        ...
        process_func = partial(process_batch, all_sequences=sequences, info_sites=info_sites, includes=includes)
        ...
        # Combine results
        dup_list.update(dup for batch_result in results for dup in batch_result)

    with open(output, "w") as f:
        for dup in sorted(dup_list):
            f.write(f"{dup}\n")
