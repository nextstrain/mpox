...
3. Add the `file-name` (without the `.ndjson` extension) as a source to `config/defaults.yaml`. This will tell the ingest pipeline to concatenate the records to the GenBank sequences and run them through the same transform pipeline.
