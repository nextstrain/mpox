...
rule notify_on_deploy:
    ...
    shell:
        """
        {params.bin_basedir}/notify-on-deploy {params.deploy_url} {params.slack_ts}
        """


onstart:
    shell(
        f"{workflow.current_basedir}/../../bin/notify-on-start {config.get('build_name', 'unknown')} {SLACK_TS_FILE}"
    )  # Saves onstart Slack message thread timestamp to file SLACK_TS_FILE


onsuccess:
    shell(f"{workflow.current_basedir}/../../bin/notify-on-success {SLACK_TS_FILE}")


onerror:
    shell(f"{workflow.current_basedir}/../../bin/notify-on-error {SLACK_TS_FILE}")
