...
rule filter:
    ...
    input:
        sequences="data/sequences.fasta",
        metadata="data/metadata.tsv",
        exclude=phylo_resolve_config_path(config["exclude"]),
    output:
...
rule combine_samples:
    input:
        ...
        include=phylo_resolve_config_path(config["include"]),
    output:
...
rule align:
    ...
    input:
        ...
        reference=phylo_resolve_config_path(config["reference"]),
...
rule mask:
    ...
    input:
        ...
        mask=phylo_resolve_config_path(config["mask"]["maskfile"]),
    output:
