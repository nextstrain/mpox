...
for name, params in config["subsample"].copy().items():
    ...
    #       non_b1: ~
    if params is None:
        print(f"Ignoring disabled subsample group config['subsample']['{name}']", file=sys.stderr)
        del config["subsample"][name]

assert len(config["subsample"]), \
...
if config.get("treefix_root", "").startswith(ROOT_FLAG):
    print(f"Removing the flag {ROOT_FLAG!r} from config['treefix_root'] ; "
